...
- **Abstention is the strongest result in the run.** Nine of the ten fully-unanswerable rows refused in all 3 runs. (Corrected in B10: I first wrote "all 10, 30 for 30" by counting `G-I11` — the *partial* insufficiency row, which has gold and should answer its federal half — as a correct refusal, while missing that `G-I06` never refuses at all. Both are failure examples in `phase_b.md`.) The refusal *rate* of 13-18% is higher than those rows alone explain because 3 to 7 answerable rows refuse per run — one consistently (G-C02), one twice, eight once. False refusals are a retrieval problem wearing an honesty costume: the sufficiency gate is right to refuse when the chunks are thin, and the chunks are thin because the plan changed.
...
## 52. Paying for the wrong axis
- **Weekly with five repeats is cheaper *and* stricter than nightly with three.** $11.65/month against $48.60, and 2.5% false passes against 6.4%. That is not a trade-off, which is the tell that the original setting was simply wrong rather than a considered compromise.
- **Frequency and repeats buy different goods.** Frequency buys detection latency. Repeats buy statistical power, directly, because the gate compares a mean and the standard error falls as 1/sqrt(n). Synthesis prompts change rarely and on purpose, so latency was worth almost nothing here — and I was paying thirty times a month for it while under-funding the thing that decides whether the gate works at all.
- **The budget check was never done.** ADR-22 recorded "$1.56 per run" and never multiplied by thirty. At nightly cadence this one job would have consumed $48.60 of a $50 monthly ceiling for the whole project. A per-run cost is not a cost; a per-month cost is.
- **Six samples told a different story than three.** Healthy 0.8715 sd 0.0127, broken 0.8415 sd 0.0154, separation 2.1 sigma rather than the 4.9 claimed from three — and the bands overlap, broken's best run beating healthy's worst. The gate still works because it compares a mean, not a single run, but the honest miss rate is one broken build in forty rather than one in three hundred.
- **Fourth time in one phase.** Three-sample estimates misled logs #46, #47, #50 and #51. The rule is now written into the ADR rather than into my intentions: re-measure both bands with at least five samples after any pipeline change.
- **Interview framing:** when a "tuning" change improves cost and quality at the same time, it was not tuning — some axis was set by habit rather than by measurement. Nightly is a default people reach for; it deserved a reason and did not have one.
- **Likely questions:** *What does your CI actually cost per month?* *How did you choose that cadence?*
