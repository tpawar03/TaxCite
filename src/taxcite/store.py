...
UPSERT = """
    ...
    VALUES (%(key)s, %(source)s, %(citation)s, %(part)s, %(section)s, %(heading)s, %(text)s, %(tokens)s,
            %(cita)s, %(as_of)s, %(fetched_at)s, %(excluded)s, %(source_revision)s)
    ON CONFLICT (key) DO UPDATE SET
        ...
        excluded = EXCLUDED.excluded, source_revision = EXCLUDED.source_revision
"""
...
def create_table(conn: psycopg.Connection) -> None:
    ...
    has_column = conn.execute(
        "SELECT 1 FROM information_schema.columns WHERE table_name = 'chunks' AND column_name = 'source_revision'"
    ).fetchone()
    if not has_column:
        conn.execute("ALTER TABLE chunks ADD COLUMN source_revision text")
