...
*Judge reliability:* Cohen's kappa **0.81** across 72 graded answers (§9.2 requires >=0.70), two judges applying the same rubric in different wording. An earlier attempt produced a misleading 0.65 before the two rubrics were aligned.
...
*Rationale:* the ranking gains exceed the recall gains — nDCG +9 points and dense MRR +13 — which matters because synthesis only sees the top few chunks. It also recovers about half of the 12.4-point regulation-recall loss that adding IRS publications caused: 0.625 → 0.688.
...
*Measurement caveat, found after this decision:* with 18 pilot questions one question is worth 5.6 points, and a tie at the k boundary made hybrid numbers vary by exactly that much between runs until ties were broken deterministically. The hybrid recall gap between these two models (+5.6) is therefore at the resolution limit of this set; **the decision rests on the deterministic dense metrics** (recall +8.3, nDCG +12.3, MRR +13.0), which were stable across runs. The 100-question golden set in §9.1 is what raises this resolution.
*Second caveat, found in B4:* the nDCG used here counted a gold citation once per retrieved chunk, and multi-part chunks share a citation, so a question could score above 1 (one pilot question scored 1.232). The **nDCG gap above is unreliable**; recall and MRR were unaffected and both favour bge-base (+8.3, +13.0), so the decision stands.
...
**Decision (Phase B, 2026-09-21): DAWSON is the Tax Court source; CourtListener is not used for it.** The fallback condition above was met on measurement (B1). CourtListener holds only ~20–60 Tax Court opinions a year since 2000, roughly the reported T.C. opinions alone. Memorandum opinions, where most hobby-loss, substantiation and home-office disputes are decided, are largely missing: a `"section 183"` search returns 108 opinions on CourtListener and 560 on DAWSON. The bulk dump doesn't help, because it is 54.6 GB across all courts with no court id on the opinion row, and the coverage is the same. *Accepted cost:* DAWSON's search runs through an undocumented endpoint behind its web app, and opinions are PDFs. The ingester rate-limits itself, sends an identifying User-Agent, and treats a changed response shape as a hard failure rather than indexing partial data. *Scope:* T.C. and memorandum opinions filed 2000 or later, limited to the Phase B topics. Summary and bench opinions are nonprecedential and excluded for now; Phase E's authority metadata is where they could return, labeled as such.
