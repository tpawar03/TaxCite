...
*Second caveat (found in B4, log #39):* the nDCG used here counted a gold citation once per retrieved chunk, and multi-part chunks share a citation, so a question could score above 1 (one pilot question scored 1.232). The **nDCG gap above is unreliable**; recall and MRR were unaffected and both favour bge-base (+8.3, +13.0), so the decision stands.
